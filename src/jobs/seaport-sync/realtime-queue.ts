...
if (config.doRealtimeWork) {
  const realtimeWorker = new Worker(
    ...
    async (job: Job) => {
      try {
        ...
        // If this is the first run initiate the backfill
        if (_.isNull(cursor)) {
          await seaportSyncBackfill.addToBackfillQueue();
        }

        newCursor = await fetchOrders(cursor);
  ...
  realtimeWorker.on("completed", async (job) => {
    ...
    const lockExtended = await extendLock("seaport-sync-lock", 60 * 5);
