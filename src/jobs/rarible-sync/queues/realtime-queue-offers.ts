...
const REALTIME_QUEUE_NAME = "realtime-rarible-sync-offers";
...
if (config.doRealtimeWork) {
    ...
    async (job: Job) => {
      try {
        ...
        if (newCursor == cursor) {
          logger.info(
            REALTIME_QUEUE_NAME,
            `Rarible offers cursor didn't change cursor=${cursor}, newCursor=${newCursor}`
          );
        }
        ...
        logger.error(
          REALTIME_QUEUE_NAME,
          `Rarible Sync offers failed attempts=${job.attemptsMade}, error=${error}`
        );
      }
    },
  ...
  realtimeWorker.on("completed", async (job) => {
    ...
    await releaseLock("rarible-sync-offers-lock", false);
