...
const BACKFILL_QUEUE_NAME = "backfill-infinity-sync";
...
if (config.doBackfillWork) {
  const backfillWorker = new Worker(
    ...
    async (job: Job) => {
      ...
      try {
        ...
        logger.info(
          BACKFILL_QUEUE_NAME,
          `Infinity backfilled from startTime=${startTime} to endTime=${endTime}`
        );
        ...
        logger.error(
          BACKFILL_QUEUE_NAME,
          `Infinity Sync failed attempts=${job.attemptsMade}, error=${error}`
        );
      }
    },
  ...
  backfillWorker.on("completed", async (job: Job) => {
    ...
    if (job.data.newStartTime) {
      await addToInfinityBackfillQueue(
...
export const addToInfinityBackfillQueue = async (
  startTime: number,
  endTime: number,
  side: "buy" | "sell",
  delayMs: number = 0
