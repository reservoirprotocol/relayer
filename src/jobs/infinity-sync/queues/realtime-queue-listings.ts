...
const REALTIME_QUEUE_NAME = "realtime-infinity-listings-sync";
  ...
  const realtimeWorker = new Worker(
    ...
    async (job: Job) => {
      try {
        ...
        const { cursor: newCursor } = await fetchOrders("sell", cursor || "");

        if (newCursor === cursor) {
          logger.info(
            REALTIME_QUEUE_NAME,
            `infinity cursor didn't change cursor=${cursor}, newCursor=${newCursor}`
          );
        }
